...
class create_task(APIView):
    ...
    def get(self, request):
        ...
        tasks_data = [{'name': task.name, 
                       ...
                       'is_choice': task.is_choice,
                       'whoCreate': task.who_create} for task in tasks]
    ...
    def delete(self, request):
        ...
        if tsk:
            task.objects.filter(description = description).first().delete()
...
class UpdateChoice(APIView):
    def post(self, request):
        ...
        if tsk:
            ...
            tsk.save()
            return Response(status = 200)
        return Response({'error' : 'Not found task in db'})